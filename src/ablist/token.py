...
class DateTimeListType:
    ...
    # Month name, e.g. January, Jan, February, Feb
    def is_month_name(self, word: str) -> bool:
        word = word.lower()

        return word in self.MONTHS or any(
            month.startswith(word) and len(word) == 3 for month in self.MONTHS
        )
    ...
    def is_weekday(self, word: str) -> bool:
        ...
        return word in self.WEEKS or any(
            day.startswith(word) and len(word) == 3 for day in self.WEEKS
        )
...
class PatternBasedListType:
    def __init__(self):
        ...
        self.CAMEL_CASE_REGEX = re.compile(r"^[A-Z][a-z]+(?:[A-Z][a-z]+)*$")

        self.SNAKE_CASE_REGEX = re.compile(r"^[a-z0-9]+(?:_[a-z0-9]+)+$")

        self.KEBAB_CASE_REGEX = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)+$")
